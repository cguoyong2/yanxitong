...
assert_json gift_offline "data.data.guestName === '${CASH_GUEST}' && data.data.blessing === '现金备注'" "offline gift should normalize guest name and blessing"
...
request favor_cash_search "${BASE_URL}/api/favor/contacts?keyword=${CASH_GUEST_Q}"
assert_json favor_cash_search "data.data.length === 1 && Number(data.data[0].receivedAmount) === 88" "offline gift was not written to favor ledger with normalized contact"
